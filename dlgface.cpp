...
void HallItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if(t->orderId.length()) {
        ...
        font.setPointSize(8);
        font.setBold(false);
        openTimeRect.adjust(openTimeRect.width() - QFontMetrics(font).horizontalAdvance(t->dateOpen) - 2, 2, -2, -2);
        painter->setFont(font);
        ...
        if(info.length()) {
            QRect comment = r;
            comment.adjust(comment.width() - QFontMetrics(font).horizontalAdvance(info) - 2, tableNameDelta, 0, 0);
...
void DlgFace::on_tblHall_clicked(const QModelIndex &index)
{
    ...
    if(!t.tryLock(tableId)) {
        DlgMessage::Msg(tr("Could not lock table"));
        return;
    }

    toTableLocked(tableId);
    t.unlock(tableId);
}
...
void ReadyDishDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    painter->drawText(rectTable, index.data(Qt::UserRole + 1).toString());
    int left = QFontMetrics(font).horizontalAdvance(index.data(Qt::UserRole + 1).toString()) + 5;
    QRect rectStaff = r;
