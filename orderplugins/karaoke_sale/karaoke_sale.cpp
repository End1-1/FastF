...
bool exec(const QMap<QString, QString> &filter, OD_Drv *o, QString &msg) {
    ...
    db.setHostName(__cnfmaindb.fHost);
    db.setDatabaseName(__cnfmaindb.fDatabase);
    db.setUserName(__cnfmaindb.fUser);
    db.setPassword(__cnfmaindb.fPassword);
    db.open();
    QSqlQuery ql(db);
    ql.prepare("select id from o_order where table_id=112 and date_close between :date1 and :date2 and state_id=2");
    ...
    if (unknownDishes.count()) {
        ...
        for (QMap<int, QString>::const_iterator i = unknownDishes.begin(); i != unknownDishes.end(); i++)
            msg += QString::fromUtf8("Անհայտ ուտեստ") + ": " + its(i.key()) + "/" + i.value() + "/" + QString::number(dishes.count()) + "\r\n";
