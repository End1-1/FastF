...
void MTPrintKitchen::printKitchen()
{
    ...
    p.ltext("_", 0);
    p.br();
    p.print(fData["printer"], QPageSize::Custom);
}
...
void MTPrintKitchen::printReceipt()
{
    ...
    p.setSceneParams(650, 2700, QPageLayout::Portrait);
    p.setFont(font);
    p.image(__cnfapp.path() + "/logo_receipt.png", Qt::AlignHCenter);
    p.br();
    /* Header */
    ...
    if(fData.contains("fiscal")) {
        QJsonObject jf = QJsonDocument::fromJson(fData["fiscal"].toUtf8()).object();
        p.ltext(jf["taxpayer"].toString(), 0);
        p.br();
        p.ltext(jf["address"].toString(), 0);
        p.br();
        p.ltext(tr("GH"), 0);
        p.rtext(jf["crn"].toString());
        p.br();
        p.ltext(tr("Tax SN"), 0);
        p.rtext(jf["sn"].toString());
        p.br();
        p.ltext(tr("Fiscal"), 0);
        p.rtext(jf["fiscal"].toString());
        p.br();
        p.ltext(tr("Tax number"), 0);
        p.rtext(QString::number(jf["rseq"].toInt()));
        p.br();
        p.ltext(tr("Tax payer"), 0);
        p.rtext(jf["tin"].toString());
        p.br();
        p.ltext(tr("(F)"), 0);
        p.br();
    }
