...
void OD_Dish::incQty(float qty)
{
    if (qty == 0.5) {
        if (!m_qty05 && f_totalQty == 1.0 && f_printedQty == 0.0)
            f_totalQty = 0.5;
        else
            f_totalQty += 0.5;
        m_qty05 = true;
...
void OD_Dish::decQty(float qty)
{
    if (qty > m_oldQty)
        return;
    if (!m_removePrinted) {
        if (qty < f_printedQty) {
            emit message(QString("%1: %2").arg(tr("You can remove only")).arg(m_oldQty - f_printedQty));
            return;
        }
    }
    f_totalQty = qty;
